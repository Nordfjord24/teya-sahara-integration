...
// Utility: create an order id and persist mapping to site/session
...
function buildCheckHash(fields: Record<string, string>, secret: string): string {
  // Implement Teya/Borgun exact field order here. Placeholder sorts keys alphabetically.
...
async function confirmDudaSession(siteName: string, sessionId: string): Promise<void> {
  // TODO: call Duda API to confirm session as PROCESSED.
  // Placeholder no-op. Intentionally minimal for sample.
  return;
}
